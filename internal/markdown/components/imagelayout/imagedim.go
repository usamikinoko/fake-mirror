...
// imageDimensions 读取本地图片文件头探测固有尺寸（像素）。
// 仅读取前 4KB，开销可忽略；解析失败（格式不支持 / 文件缺失 / 远程图）返回 ok=false。
func imageDimensions(link string) (w, h int, ok bool) {
	if strings.HasPrefix(link, "data:") {
		return 0, 0, false
	}
	if strings.HasPrefix(link, imageRootURL) {
		link = imageRootDir + strings.TrimPrefix(link, imageRootURL)
	}
	return dimsFromFile(link)
}

func dimsFromFile(path string) (w, h int, ok bool) {
